...
- Ensure the PR is conflict-free with staging.
...
4. Check mergeability and conflicts against staging.
5. If conflicts exist, use the `pull` skill to fetch/merge `origin/staging` and
...
- If CI pushes an auto-fix commit (authored by GitHub Actions), it does not
  ...
  `origin/staging` if needed, add a real author commit, and force-push to retrigger
  CI, then restart the checks loop.
- If all jobs fail with corrupted pnpm lockfile errors on the merge commit, the
  remediation is to fetch latest `origin/staging`, merge, force-push, and rerun CI.
