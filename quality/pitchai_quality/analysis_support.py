...
def registered_route(node: ast.FunctionDef | ast.AsyncFunctionDef, tree: ast.Module) -> bool:
    """FastAPI registration consumes the callable even without a Python caller."""
        ...
        if isinstance(statement, ast.Import):
            for alias in statement.names:
                if alias.name == "fastapi":
                    constructors.update(f"{alias.asname or alias.name}.{name}" for name in ("FastAPI", "APIRouter"))
    ...
    for statement in tree.body:
        ...
        created = isinstance(value, ast.Call) and ast.unparse(value.func) in constructors
        for target in targets:
            if isinstance(target, ast.Name):
                receivers.discard(target.id)
                if created:
                    receivers.add(target.id)
    methods = {"get", "post", "put", "patch", "delete", "head", "options", "trace", "api_route", "websocket"}
    return any(
