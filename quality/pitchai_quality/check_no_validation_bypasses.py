...
_EXPECTED_PORTABLE_MANIFEST_SHA256 = "39e180988ba6d6aacea9abcef9ed3a1372ecaf9ec15bbbc3c4b8de6860a7dcb2"
...
_DIRECT_TOOLCHAIN = re.compile(
    r"\b(?:ruff|basedpyright|pyright|pylint|semgrep)\b|uv\s+run[^\n]*\b(?:quality|check)\b|pitchai_quality\.check",
    re.IGNORECASE,
)
...
def _inline_violations() -> list[Violation]:
    ...
    for path in (*iter_python_files((REPOSITORY_ROOT,)), *_CONFIG_PATHS):
        ...
        lines: Iterable[tuple[int, str]] = enumerate(source.splitlines(), start=1)
        if path.suffix in {".py", ".pyi"}:
            tokens = tokenize.generate_tokens(io.StringIO(source).readline)
            lines = ((token.start[0], token.string) for token in tokens if token.type == tokenize.COMMENT)
...
def _workflow_and_toolchain_violations() -> list[Violation]:
    ...
    for name in root_tool_sections(root_pyproject):
        violations.append(Violation(root_pyproject, 1, f"alternate root tool.{name} configuration is forbidden"))
    for relative in CONFLICTING_ROOT_CONFIGS:
        path = REPOSITORY_ROOT / relative
        if path.exists():
