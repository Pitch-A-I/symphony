defmodule SymphonyElixir.CoreTest do
  ...
  test "config defaults and validation checks" do
    ...
    assert config.tracker.terminal_states == ["Done", "Cancelled", "Canceled", "Duplicate"]
  ...
  test "current WORKFLOW.md file is valid and complete" do
    ...
    assert Map.get(tracker, "kind") == "pitchai_pm"
    ...
    assert is_map(hooks)
    assert Map.get(hooks, "after_create") =~ "bootstrap_pitchai_pm_workspace.sh"
    assert Map.get(hooks, "before_run") =~ "bootstrap_pitchai_pm_workspace.sh"
  ...
  defp assert_due_after(due_at_ms, base_ms, min_delay_ms, max_delay_ms) do
    delay_ms = due_at_ms - base_ms
  ...
  test "prompt builder uses a sensible default template when workflow prompt is blank" do
    ...
    assert prompt =~ "You are working on a PitchAI project-management task."
  ...
  test "in-repo WORKFLOW.md renders correctly" do
    ...
    assert prompt =~ "Do not include vague next steps for the user."
  ...
  test "app server starts with workspace cwd and expected startup command" do
    ...
    try do
      ...
      assert String.contains?(argv_line, "--yolo app-server")
  ...
  test "app server startup command supports codex args override from workflow config" do
    ...
    try do
      ...
      assert argv_line = Enum.find(lines, fn line -> String.starts_with?(line, "ARGV:") end)
      assert String.contains?(argv_line, "--config model=\"gpt-5.5\" --yolo app-server")
